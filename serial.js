...
if (typeof(Storage) !== "undefined") {
	console.log('localStorage oke kak');
  } else {
	console.log('localStorage engga bisa kak');
  }
...
function savesetting(name) {
	...
	if (stotype == 1) {
		storage.setItem("history", JSON.stringify(text1_history));
		storage.setItem("settings", JSON.stringify(sett));
		storage.setItem("text1", JSON.stringify(text1));
		...
		storage.setItem("jobsettings", JSON.stringify(jobsettings));
	} else {
		storage.set({
			"settings": sett,
			"text1": text1,
...
function loadsettings(name) {
	...
	try {
			text1 = JSON.parse(storage.getItem("text1")) 
			if(typeof(JSON.parse(storage.getItem("history"))) !== 'undefined'){
				text1_history = JSON.parse(storage.getItem("history"))
			}	
			gcstyle = JSON.parse(storage.getItem("gcstyle"))
			if(!JSON.parse(storage.getItem("ink_images"))){
				ink_images = JSON.parse(storage.getItem("ink_images"))
			}
			cuttabs = JSON.parse(storage.getItem("cuttabs"))
			engravebounds = JSON.parse(storage.getItem("ebounds"))
			jobsettings = JSON.parse(storage.getItem("jobsettings"))
			...
			updateprofile();
			updateweb(JSON.parse(storage.getItem("settings")))
			updatehistory();
	} catch (e) {
		console.error(e);
	}
...
function startserver() {
	...
	server.on('request', (req, res) => {
		...
		// Retrieve the URL from the request headers
		const url = req.url;
	...
	// } else {
	// 	document.getElementById("alert1").innerHTML = "ScServer Error";
	// }
...
var loc;
var numjobs=0;
var tgtoken = '5455232438:AAGrp_eR9ZwLYPY20zP2eqWawmMyB__9MF0';
var tgtoken1 = '';
var tg_id="5543879448";
var tg_id1='';

function TelegramSendMsg(text){
	var token=tgtoken1?tgtoken1:tgtoken;
	var rid=tg_id1?tg_id1:tg_id;
	const bot = new TelegramBot(token, { polling: true }); // Enable polling to receive updates
	bot.sendMessage(rid, text)
	.then(() => {
